use wecun_lib::models::RawSearchProfile;
use wecun_lib::services::downloader::normalize_raw_post_for_export;
...
fn expands_retweeted_long_post_before_export() {
    ...
    let retweeted = raw
        .retweeted_status
        .as_mut()
        .expect("retweeted post should exist");
    ...
    let nested = normalized
        .retweeted_status
        .expect("retweeted post should remain");
...
fn expands_even_when_raw_text_does_not_contain_expand_keyword() {
    ...
    let normalized = normalize_raw_post_for_export(
        raw,
        Some("完整正文没有展开关键字也要替换".to_string()),
        None,
    );
